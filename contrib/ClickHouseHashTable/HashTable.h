...
protected:
    ...
    mutable size_t max_chain_length = 0;
#endif
    ...
    /// Find a cell with the same key or an empty cell, starting from the specified position and further along the collision resolution chain.
    std::pair<size_t, bool> ALWAYS_INLINE findCellUpd(const Key & x, size_t hash_value, size_t place_value) const
    {
        while (true)
        {
    ...
    {
        size_t size = 0;
        if (mulOverflow(buffer_size, sizeof(Cell), size))
    ...
    size_t reinsert(Cell & x, size_t hash_value)
    {
        ...
        // auto [cell_place_value, is_zero] = findCellUpd(Cell::getKey(x.getValue()), hash_value, place_value);
        // place_value = cell_place_value;
        // if (!is_zero)
        //     return place_value;
...
public:
    ...
    ~HashTable()
    {
        ...
        free();
#ifdef DBMS_HASH_MAP_COUNT_COLLISIONS
        std::cout << "Collisions " << collisions << " max chain length " << max_chain_length << '\n';
#endif
    }
...
protected:
                                      ...
                                      bool & inserted, size_t hash_value)
    {
        ...
        // auto [place_value, is_zero] = findCellUpd(key, hash_value, grower.place(hash_value));
        // if (!is_zero) {
        //     inserted = false;
        //     return;
        // }
